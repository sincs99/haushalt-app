...
**Datei:** `backend/app/routers/polls.py:171–179`
        ...
        if recipe is None or recipe.household_id != household_id:
            raise HTTPException(
                ...
                detail=error_detail(
                    ...
                    f"Recipe not found in this household",
                ),
            )
    option = EventPollOption(
        poll_id=poll.id,
...
**Datei:** `backend/app/routers/food.py:31` (RecipeCreate), `food.py:40` (RecipeUpdate)
...
{
  "name": "Evil Recipe",
  "ingredients": ["AAAA....(1 MB String)"]
}
```
Oder 10.000 kurze Strings → DB-Bloat, langsame JSON-Serialisierung, Memory-Pressure.
...
**Datei:** `backend/app/routers/food.py:453–454`
...
### Korrekturvorschlag
Wird durch F-2 (Ingredient-Validierung mit max 200 Zeichen) automatisch behoben. Zusätzlich als Defense-in-Depth:
...
**Datei:** `backend/app/routers/food.py:401–500`  
**Kontext:** `backend/app/main.py` — kein globales Rate-Limiting
...
**Datei:** `backend/app/routers/polls.py:52`
